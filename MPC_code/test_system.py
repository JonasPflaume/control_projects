...
ax[0].set_title('Above:no control and random init. states output\nBelow:3 times responses from the same step signal')
...
#np.random.seed(0)
#uRandom = np.random.randn(time.shape[0]-1,2) # Random forcing input
#u=uRandom
...
# Concatenate:
    ...
    ax[1].set_title('')
plt.savefig('Step_responce_on_system',dpi=250)
plt.show()
